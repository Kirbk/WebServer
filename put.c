...
int put_m(char ** message, http_request_header* request_h, http_response_header* response_h) {
    ...
    int ret = 405; //exists ? 201 : 204; // Put method allowed in configuration.
    ...
    *response_h = create_http_response_header();
    ...
    FILE * update = fopen(resource, "w");
    int written = fwrite(request_h->message, strlen(request_h->message), 1, update);
    if (written != strlen(request_h->message)) ret = 500;
    fclose(update);
    update = NULL;
